...
def main():
	...
	for mem in Ein:
		for j in range(1, length - 1):
			if mem[3] == 1: 
				G[x_1[j][2]] += mem[4] * decisionStump(mem[1], x_1[j][0], mem[2])
			else:
				G[x_2[j][2]] += mem[4] * decisionStump(mem[1], x_2[j][0], mem[2])
		error = 0.0
		for j in range(1, length - 1):
			if x_1[j][1] != sign(G[x_1[j][2]]):
				error += 1.0
		Ein_Gt.append(error/mother)

	print "Ein(G) = ", Ein_Gt[len(Ein_Gt)-1]
