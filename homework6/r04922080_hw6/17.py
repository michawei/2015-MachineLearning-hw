...
def main():
	...
	for line in f:
		...
		if line == '':
			break
		x_1_test.append([float(line[0]), float(line[2]), index])
		x_2_test.append([float(line[1]), float(line[2]), index])
		index += 1
	f.close()
	...
	for mem in Ein:
		...
		for j in range(len(x_1_test)):
			if mem[3] == 1: 
				if x_1_test[j][1] != decisionStump(mem[1], x_1_test[j][0], mem[2]):
					error += 1.0
			else:
				if x_2_test[j][1] != decisionStump(mem[1], x_2_test[j][0], mem[2]):
					error += 1.0
